...
dir_oecd_project <- ("A:/projects/brlanduse_landscape102023/oecd")

dir_oecd_output_2023 <- ("A:/projects/brlanduse_landscape102023/oecd/output")
...
setwd(dir_oecd_output_2023)

df_ocde_filter <- readRDS("ocde_filter_replication.rds")

# df_ocde_filter <- readRDS("ocde_filter_reviewed.rds")

setwd(dir_oecd_project)

source_ocde <- read.xlsx("10_oecd_relational_tables.xlsx", sheet = "source_landscape") %>% 
  unique() %>% 
  select(-provider_type, -provider_detailed, -extending_agency)

channel_ocde <- read.xlsx("10_oecd_relational_tables.xlsx", sheet = "channel_landscape") %>% 
  dplyr::mutate(channel_original = tolower(channel_original))

instrument_ocde <- read.xlsx("10_oecd_relational_tables.xlsx", sheet = "instrument_landscape") %>% 
...
sector_ocde <- read.xlsx("10_oecd_relational_tables.xlsx", sheet = "sector_landscape")
...
df_ocde_transform <-  left_join(df_ocde_transform, channel_ocde, by ="channel_original")
sum(df_ocde_transform$climate_related_development_finance_commitment_current_usd_thousand)
...
df_ocde_transform <- inner_join(df_ocde_transform,instrument_ocde, by = "instrument_original")

df_ocde_transform <- df_ocde_transform %>% dplyr::mutate(channel_landscape = ifelse(is.na(channel_landscape),"-",channel_landscape))

df_ocde_transform <- left_join(df_ocde_transform, data_subsector %>% select(purpose_code, clarifications), by = "purpose_code")
...
df_ocde_calculus2 <- deflate_and_exchange(tabela_deflator, df_ocde_transform, tabela_cambio)

#### validate data sector

rm(cambio_sgs,df_ocde_transform, ibge_ipca, tabela_cambio, tabela_deflator, teste)

df_ocde_calculus2 <- df_ocde_calculus2 %>% 
...
write.xlsx(df_ocde_calculus2,"df_ocde_landscape_final_2020.xlsx")

saveRDS(df_ocde_calculus2,"df_ocde_landscape_final_2020.rds")
