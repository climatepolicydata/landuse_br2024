...
df_sicor <- df_sicor %>% select(-cesta,
                                ...
                                -cd_tipo_encarg_financ,-data_inicio,-
                                  cd_ref_bacen_investimeto,-unidade_medida)
#### ---------------------------------------------------------------------- ####
...
df_sicor <- df_sicor %>% select(-UNIDADE_MEDIDA)
...
df_sicor <- df_sicor %>%
  group_by(ANO, ATIVIDADE,CODIGO_FINALIDADE, CD_PROGRAMA,
...
df_sicor_aggregate <- aggregate(VL_PARC_CREDITO ~ ATIVIDADE+CODIGO_FINALIDADE+ CD_PROGRAMA+
                                CD_SUBPROGRAMA+ CD_FONTE_RECURSO+ CD_TIPO_INTGR_CONSOR+
                                CD_TIPO_AGRICULTURA+ CODIGO_PRODUTO+ CODIGO_MODALIDADE+
...
saveRDS(df_sicor, "df_sicor_op_basica_pre_dummie_190124.RDS")
